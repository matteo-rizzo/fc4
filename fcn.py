...
from auxiliary.settings import initialize_dataset_config, INPUT_GAMMA, FCN_INPUT_SIZE, PATH_TO_MODEL, OVERRODE
...
class FCN:
    ...
    def build_branches(images, dropout):
        ...
        Images are 0-65535 linear RGB

        There are two ways to do confidence-weighted pooling
          1. Output (normalized) R, G, B and weight, average RGB by weight
          2. Just output non-normalized R, G, B, and simply take the sum and normalize (weighting using the length)
        """
        ...
        if FC_POOLING:
            ...
            fc2 = tf.nn.l2_normalize(slim.conv2d(tf.nn.relu(fc2),
                                                 3, [15, 15],
                                                 scope='fc_pooling',
                                                 activation_fn=None,
                                                 padding='VALID'), 3)
    ...
    def build(self):
        ...
        self.per_patch_weight = tf.placeholder(tf.float32, shape=(), name='pre_patch_weight')
        ...
        self.global_loss = self.get_angular_loss(tf.reduce_sum(fc2, axis=(1, 2)),
                                                 self.illuminants,
                                                 LENGTH_REGULARIZATION)

        self.per_patch_loss = self.get_angular_loss(fc2, self.illuminants[:, None, None, :], LENGTH_REGULARIZATION)

        self.loss = (1 - per_patch_weight) * self.global_loss + self.per_patch_weight * self.per_patch_loss

        scalar_summaries = []
        scalar_summaries.append(tf.summary.scalar('per_patch_loss', self.per_patch_loss))
        scalar_summaries.append(tf.summary.scalar('full_image_loss', self.global_loss))
        ...
        var_list1 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='FCN/AlexNet')
        var_list2 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='FCN/fc1') + \
                    tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='FCN/fc2')
    ...
    def build_shallow(images):
        net = images
        net = slim.conv2d(net, SHALLOW_CHANNELS[0], [11, 11], stride=4, scope='conv1', padding='SAME')
        net = slim.max_pool2d(net, [3, 3], padding='VALID')
        net = slim.conv2d(net, SHALLOW_CHANNELS[1], [5, 5], stride=1, scope='conv2', padding='SAME')
        net = slim.max_pool2d(net, [3, 3], padding='VALID')
        net = slim.max_pool2d(net, [3, 3], padding='VALID')
        return net
    ...
    def test_multi(self, summary=False, summary_key=0):
        ...
        for r in records:
            ...
            illuminant = r.illuminant.astype(np.float32)
            pixels = cv2.resize(pixels, illuminant.shape[0:2][::-1])
            pixels /= np.linalg.norm(pixels, axis=2)[:, :, None]
            illuminant /= np.linalg.norm(illuminant, axis=2)[:, :, None]

            pixels = pixels[:, :, ::-1]

            cv2.imshow('pixels', cv2.resize(pixels, (0, 0), fx=2, fy=2))
            cv2.imshow('illuminant', cv2.resize(illuminant, (0, 0), fx=2, fy=2))
            cv2.imshow('image', img)
            cv2.waitKey(0)

            illuminant = illuminant.astype(np.float32)
            pixels = pixels.astype(np.float32)

            dot = np.sum(pixels * illuminant, axis=2)
    ...
    def test_naive():
        ...
        for r in records:
            ...
            # print(r.file_name, est)
            # est=np.array((1, 1, 1))
            # est2= std['estimated_illuminants'][names.index(r.file_name[:-4])]
            gt2 = std['groundtruth_illuminants'][names.index(r.file_name[:-4])]
            # print(est2)
    ...
    def inference(self, file_name, datapacks=['g0', 'g1', 'g2']):
        ...
        for r in records:
            ...
            pixels = pixels[0]
            est = est[0]
            return img, pixels, est, r.illuminant
        ...
        for r in records:
            ...
            for scale, weight in zip(scales, weights):
                ...
                if shape not in self.test_nets:
                    ...
                    test_net = {}
                    test_net['illuminants'] = tf.placeholder(
        ...
        if summary:
            for file_name, error, merged in summaries:
                ...
                try:
                    ...
                    pass
                summary_fn = '%s/%5.3f-%s.png' % (folder, error, file_name)
                cv2.imwrite(summary_fn, merged[:, :, ::-1] * 255)
    ...
    def test_external(self, images, scale=1.0, fns=None, show=True, write=True):
        ...
        for img, filename in zip(images, fns):
            ...
            indices = int(confidences.flatten().shape[0] * 0.95)
            print(confidences.mean(), confidences.max(),
                  sorted(confidences.flatten())[indices])
            merged = merged[0]
            illuminants.append(est)
        ...
        return illuminants, confidence_maps
        ...
        for r in records:
            ...
            illum_est = self.sess.run(
                ...
                feed_dict={self.images: [img],
                           self.dropout: 1.0})
            avg_error = math.degrees(angular_error(illum_est, r.illuminant))
    ...
    def test_patch_based(self, scale, patches, pooling='median'):
        ...
        for r in records:
            ...
            avg_error = math.degrees(angular_error(illum_est_avg, r.illuminant))
            median_error = math.degrees(angular_error(illum_est_median, r.illuminant))
    ...
    def get_ckpt_folder(self, name=None):
        if name is None:
            name = self.name
        return PATH_TO_MODEL + '/' + name + '/'

    def backup_scripts(self):
        for file_name in os.listdir('.'):
            if file_name.endswith('.py'):
                shutil.copy(file_name, self.get_ckpt_folder())
    ...
    def load(self, key, name=None):
        file_name = self.get_filename(key, name=name)
        self.load_absolute(file_name)

    def load_absolute(self, file_name):
        self.saver.restore(self.sess, file_name)
        print("Model %s restored." % file_name)
